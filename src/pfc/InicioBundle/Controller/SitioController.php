...
//        $defaultData = array('codigo' => '');
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                ...
                $datos = $form->getData('codigo');  // datos es un array con clave: "codigo" 
                //TUVYdfuIrTHnMATlIPQmYcQg%2BGZHvjLirxDEYS9rru45vxBgH6kijnhPVaOIifOoG06szxlW1%2FxZpToMPYOuY3H777NPMutQOz7nx6E9lZQ2%2BX0qkpaYwK3iv9xGcedOJ7witOztuCr5La5EQXtK86Pl8SYpulFQXSbVot8yOfCcbg%3D%3D
                
                //desencripta cadena    
                $codigo = urldecode($datos['codigo']);
                $codigo = base64_decode($codigo);
                $x2 = $codigo;
                $clave = substr($codigo, 0, 2);
                $datos = substr($codigo, 2);
                $datos = $this->desencriptar($datos, $clave);
//                return new Response($x2.'<pre>' . $datos . '<br />' . $clave . ' - ' . $codigo . print_r($datos, true) . '</pre>');
                if(count(explode('·', $datos)) == 3 ){
                    list($a,$b,$c) = explode('·', $datos);
                    if(!is_numeric($a) || !date('Y-m-d H:i:s',$b)){ throw $this->createNotFoundException('Código no válido.'); }
                }else{                                              throw $this->createNotFoundException('Código inválido.');}
                $formato='Y-m-d H:i:s';
...
//                return new Response('REPOSITORY<pre>'.print_r($usuario, true) . '</pre>');
                ...
                if($usuario){
                    ...
                    $token = new UsernamePasswordToken(
                            $usuario, $usuario->getPassword(), 'chain_provider', $usuario->getRoles()
                    );
                    $this->container->get('security.context')->setToken($token);
                }
                else{throw $this->createNotFoundException('Código ya usado.');}

                return $this->redirect($this->generateUrl('portada'));
            }
            return $this->redirect($this->generateUrl('activar_cuenta'));
        }
        return $this->render('InicioBundle:Sitio:activar.html.twig'
...
//            return new Response('real:'.$resultado_real.' respondido:'.$resultado_respondido.' opreacion:'.$operacion);
...
//                $this->get('session')->setFlash('info','¡Enhorabuena! Te has registrado correctamente en PFC');
                $this->get('session')->getFlashBag()->add('info',$datos. 'Se le ha enviado un correo con el código de activación de cuenta.'.$codigo);
